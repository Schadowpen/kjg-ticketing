...
class PdfFile {
     ...
     *
     * @param string|StringReader $pdfSource Name und Pfad der PDF-Datei als String oder StringReader mit PDF-Inhalt
     *
     * @throws \Exception Wenn etwas mit der PDF-Datei nicht in Ordnung ist
     */
    public function __construct( $pdfSource ) {
        try {
            if ( $pdfSource instanceof StringReader ) {
                ...
                $this->originalFileContent = new StringReader( $fileString );
            }
    ...
    protected function extractPdfFileEnd( $startOfXref ) {
        ...
        $stringReader = new StringReader( $crossReferenceStream->getDecompressedStream() ); // StringReader liest nun Stream aus
    ...
    private function readObjectStream( CrossReferenceTableEntry $crossReferenceTableEntry ) {
        ...
        $objectPositionParser = new ObjectParser( new StringReader( $streamString ), 0 );
        $objectParser = new ObjectParser( new StringReader( $streamString ) );
    ...
    public function savePdfFile( bool $allowDataCompression = true ) {
        ...
        if ( $useObjectStreams ) {
            ...
            $crossReferenceStreamDictionary->setObject( "W", new PdfArray( [
                new PdfNumber( $typeByteSize ),
                new PdfNumber( $field2ByteSize ),
                new PdfNumber( $field3ByteSize )
            ] ) );
     ...
     * @return StringReader
     */
    public function getOriginalFileContent(): StringReader {
    ...
    public function parseReference( ?PdfAbstractObject $pdfObject ): ?PdfAbstractObject {
        ...
        if ( $indirectObject === null ) {
            return new PdfNull();
        } // wenn kein Objekt existiert, soll das referenzierte Objekt als null angenommen werden
