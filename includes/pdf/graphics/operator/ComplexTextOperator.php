...
class ComplexTextOperator extends AbstractTextOperator {
    ...
    public function __construct( PdfNumber $wordSpacing, PdfNumber $characterSpacing, PdfAbstractObject $text, OperatorMetadata $operatorMetadata = null ) {
        ...
        if ( ! ( $text instanceof PdfString ) && ! ( $text instanceof PdfHexString ) ) {
            ...
            throw new \Exception( "Argument 1 passed to KjG_Ticketing\pdf\graphics\operator\ComplexTextOperator::__construct() must be an instance of KjG_Ticketing\pdf\object\PdfString or KjG_Ticketing\pdf\object\PdfHexString, instance of {$textClassName} given" );
        }
        $this->text = $text;
    }
