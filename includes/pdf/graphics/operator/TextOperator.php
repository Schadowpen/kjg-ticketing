...
class TextOperator extends AbstractTextOperator {
    ...
    public function __construct( PdfAbstractObject $text, OperatorMetadata $operatorMetadata = null ) {
        ...
        if ( ! ( $text instanceof PdfString ) && ! ( $text instanceof PdfHexString ) ) {
            ...
            throw new \Exception( "Argument 1 passed to pdf\graphics\operator\TextOperator::__construct() must be an instance of pdf\object\PdfString or pdf\object\PdfHexString, instance of {$textClassName} given" );
        }
        $this->text = $text;
    }
